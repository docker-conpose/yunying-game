...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
	...
	public OrderInfo orderSub(PlaceOrderDTO placeOrderDTO) {
		...
		List<GoodsCategory> listGoodsSpu = new ArrayList<>();
		placeOrderDTO.getSkus().forEach(orderGoods -> {//过滤
			GoodsCategory goodsSpu =  goodsCategoryService.getOne(Wrappers.<GoodsCategory>lambdaQuery()
					.eq(GoodsCategory::getId, orderGoods.getSpuId())
					.eq(GoodsCategory::getEnable, "1")
					.ge(GoodsCategory::getStock, orderGoods.getQuantity()));
			if(goodsSpu != null){
				OrderItem orderItem = new OrderItem();
				orderItem.setOrderId(orderInfo.getId());
				orderItem.setSpuId(goodsSpu.getId());
				orderItem.setSpuName(goodsSpu.getName());
				orderItem.setPicUrl(goodsSpu.getPicUrl());
		...
		if(listOrderItem.size() > 0){
			...
			//更新库存
			listGoodsSpu.forEach(goodsCategoryService::updateById);
			//订单自动取消时间
	...
	@Override
	public int orderTotal(OrderInfo orderInfo) {
		return baseMapper.selectCount(Wrappers.<OrderInfo>lambdaQuery().eq(OrderInfo::getDelFlag,0)
				.eq(StrUtil.isNotBlank(orderInfo.getStatus()),OrderInfo::getStatus,orderInfo.getStatus()));
	}
